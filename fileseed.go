...
func NewIndexSeed(dstFile string, srcFile string, index desync.Index) (*FileSeed, error) {
	s := FileSeed{
		srcFile:    srcFile,
		pos:        make(map[desync.ChunkID][]int),
		index:      index,
		canReflink: desync.CanClone(dstFile, srcFile),
		isInvalid:  false,
	}
...
func (s *FileSeed) LongestMatchWith(chunks []desync.IndexChunk) (int, desync.SeedSegment) {
	s.mu.RLock()
...
func (s *FileSeed) RegenerateIndex(ctx context.Context, n int, attempt int, seedNumber int) error {
	...
	s.index = index
	s.SetInvalid(false)
	s.pos = make(map[desync.ChunkID][]int, len(s.index.Chunks))
	for i, c := range s.index.Chunks {
		s.pos[c.ID] = append(s.pos[c.ID], i)
	}
...
func (s *FileSeed) maxMatchFrom(chunks []desync.IndexChunk, p int, limit int) []desync.IndexChunk {
	if len(chunks) == 0 {
		return nil
	}
	var (
...
func newFileSeedSegment(file string, chunks []desync.IndexChunk, canReflink bool) *fileSeedSegment {
...
func (s *fileSeedSegment) clone(dst, src *os.File, srcOffset, srcLength, dstOffset, blocksize uint64) (uint64, uint64, error) {
	...
	return copied, alignLength, desync.CloneRange(dst, src, srcAlignStart, alignLength, dstAlignStart)
}
...
func (s *fileSeedSegment) Reader() (reader io.ReadCloser, err error) {
	...
	return &SectionReaderCloser{
		io.NewSectionReader(file, int64(s.chunks[0].Start), int64(s.Size())),
